...
with st.sidebar:
    ...
    if st.session_state["profile_data"] is None:
        signin_button = st.button("🔑 Sign In", use_container_width=True, type="primary")
        st.markdown("*Please sign in to start using Clare-AI*")
    else:
        profile_button = st.button("✏️ Edit Profile", use_container_width=True)
    ...
    if st.session_state["profile_data"] is None and signin_button:
        st.session_state["show_profile_form"] = True
        st.rerun()
